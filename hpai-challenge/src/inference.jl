...
"""
    find_map_full_reparam(data, in_zone; n_samples=1000) -> NamedTuple

Random search from prior for the reparameterised 8-parameter model.
Searches over (t₀, φ_hrz, ρ_non, δ, β_duck, κ, α, p_mov).
"""
function find_map_full_reparam(data::ModelData, in_zone::BitMatrix; n_samples::Int = 1000)
    best_ll = -Inf
    best_params = nothing
    ...
    for _ in 1:n_samples
        ...
        # Derived parameters for likelihood
        φ_non = ρ_non * φ_hrz
        β_val = κ * φ_hrz
        ...
        lp = logpdf(d_t₀, t₀) + logpdf(d_φ_hrz, φ_hrz) +
             logpdf(d_ρ_non, ρ_non) + logpdf(d_δ, δ) +
             logpdf(d_β_duck, β_duck) + logpdf(d_κ, κ) +
             logpdf(d_α, α) + logpdf(d_p_mov, p_mov)
        total = ll + lp
        if total > best_ll
            best_ll = total
            best_params = (t₀=t₀, φ_hrz=φ_hrz, ρ_non=ρ_non, δ=δ,
                           β_duck=β_duck, κ=κ, α=α, p_mov=p_mov)
        end
    end

    if best_params === nothing
        error("MAP reparam search found no finite posterior evaluations. " *
              "Check data and priors.")
    end
    println("MAP reparam: log-posterior = $(round(best_ll, digits=2))")
    println("  t₀=$(round(best_params.t₀, digits=1)), " *
            "φ_hrz=$(round(best_params.φ_hrz, sigdigits=3)), " *
            "ρ_non=$(round(best_params.ρ_non, sigdigits=3)), " *
            "δ=$(round(best_params.δ, sigdigits=3)), " *
            "β_duck=$(round(best_params.β_duck, digits=3)), " *
            "κ=$(round(best_params.κ, sigdigits=3)), " *
            "α=$(round(best_params.α, digits=0)), " *
            "p_mov=$(round(best_params.p_mov, digits=4))")
    return best_params
end
